...
dirmapas <- "~/Documents/DataBase/Mapas1/2012_nacional_provincias"
setwd(dirmapas)
...
############## Todos menos Gal?pagos y Zonas No delimitadas:
poligonos <- subset(poligonos,poligonos@data$DPA_PROVIN=="01" |
                      poligonos@data$DPA_PROVIN=="02" | poligonos@data$DPA_PROVIN=="03" |
                      poligonos@data$DPA_PROVIN=="04" | poligonos@data$DPA_PROVIN=="05" |
                      poligonos@data$DPA_PROVIN=="06" | poligonos@data$DPA_PROVIN=="07" |
                      poligonos@data$DPA_PROVIN=="08" | poligonos@data$DPA_PROVIN=="09" |
                      poligonos@data$DPA_PROVIN=="10" | poligonos@data$DPA_PROVIN=="11" |
                      poligonos@data$DPA_PROVIN=="12" | poligonos@data$DPA_PROVIN=="13" |
                      poligonos@data$DPA_PROVIN=="14" | poligonos@data$DPA_PROVIN=="15" |
                      poligonos@data$DPA_PROVIN=="16" | poligonos@data$DPA_PROVIN=="17" |
                      poligonos@data$DPA_PROVIN=="18" | poligonos@data$DPA_PROVIN=="19" |
                      poligonos@data$DPA_PROVIN=="21" | poligonos@data$DPA_PROVIN=="22" |
                      poligonos@data$DPA_PROVIN=="23" | poligonos@data$DPA_PROVIN=="24" )
...
postscript("MapaAp1.ps", width=5.5+2, height=3.9+2, pointsize=8, 
           horizontal=FALSE)
plot(poligonos)
...
"Los nombres de las Provincias y sus ubicaciones en el vector"
"CatNam: ProNam"
"CatLoc: ProNom"
...
ProRem <- function(cm, Mat)
{
  ...
  "  cm   Numero de fila de nombre (acr?nimo de la Prov)"
...
##  datos: Datos del Censo de Poblaci?n y Vivienda
##  A?o: 2010
...
LlarC <- as.matrix(datos[,3:10])
rownames(LlarC) <- AcrProv

##  Chequeo el calculo de la Proporcion de Indigenas
LlarH <- LlarC[,1]/LlarC[,c(seq(8))] %*% matrix(rep(1,8))
...
title(main = "Proporcion de Indigenas",cex=3)
...
graphics.off()
postscript("MapaEcuInd.ps", width=5.5+2, height=3.9+2, pointsize=8, 
           horizontal=FALSE)
plot (poligonos.data, col=colcode, border="grey", axes=T)
#title(main = "Proporcion de Indigenas",cex=3)
legend("bottomright",legend = names(attr(colcode,"table")),
...
postscript("Fig2Ap1.ps", width=5.5, height=3.9, pointsize=8, 
           horizontal=FALSE)
...
barplot((LlarC[,1]),space=0,col=rainbow(nProv),
        xlab="Provincias", ylab="Numero Indigenas",cex.names=0.6)
barplot(as.vector(LlarH),space=0,col=rainbow(nProv),
        xlab="Provincias", ylab="Porcentaje de Indigenas",cex.names=0.6)
...
NParc <- function(nD=round(CatC07/4000), B2=seq(0,0.03,length=101), B2m=0.02)
{
  ...
  plot(c(0,D+1), range(c(1,nD)), type="n", 
       ...
       sub="(orden ascendente del tamano de la poblacion)", 
...
NParcR <- NParc(nD=round(LlarC[,1]/200), 
                B2=seq(0,35, length=101), B2m=c(25, 9))

legend(0,1100, pch=c(-1,6,4,1), c("Pesos", "Prop. al tamano de n", 
...
{
  ##   Funcion para comparar dos vectores con entradas positivas y del mismo tamano
  ##   Uso princial:  salida de la funcion SAcs
  ##   vc1,  vc2    vectores
  ##   dgt      Numero de d?gitos para el redondeo 
...
##  Estimacion de area pequena para las proporciones de indigenas
...
SSRSe <- function(tbl, fra, ctg, vls)
{
  ...
  ##  La proporcion de observaciones en las categoroas objetivo
...
for (i in seq(n.sim))
  SSrwV[[i]] <- apply(LlarC[,1:8], 1, SSRSe, fra=1/200, ctg=1, vls=rep(1,8))
...
SAcsFb <- function(drE, trg=SAproTrg, Psz=apply(LlarC,1,sum), Dmat=ProSim, mxD=3)
{
  ...
  ##    trg     Los objetivos
  ##    Psz     Los tama?os poblacionales de los distritos
  ...
  for (i in seq(nD))
  {
    ##  Los tama?os de la poblacion para los h-anillos (denominadores)
    ...
    ##  Estimacion de la desviaci?n al cuadrado de los h-anillos del objetivo
    EvdH <- 0
...
setwd(dir.dat)
datos01 <- read.csv("datosCPV01.csv",sep=";",header=TRUE)
...
############################################################################################
############################################################################################
##  Estimacion compuesta bivariada (simetria compuesta)
##  Estimacion de las proporciones con informacion auxiliar del censo


SAcs2Ab <- function(drE, Aux=Llar01[,1]/apply(Llar01,1,sum), trg=SAproTrg, tkn=0)
{
  ##  Estimacion compuesta bivariada (simetria compuesta)
...
SAcs2Fb <- function(drE, Aux=Llar01[,1]/apply(Llar01,1,sum), trg=SAproTrg, 
                    Psz=apply(LlarC,1,sum), Dmat=ProSim, tkn=0.00001, mxD=3)
{
  ...
  ##    trg      Los objetivos
  ##    Psz     Tama?os de la poblacion por distrito
  ...
  ##  Los errore de estimacion
  theDS - trg
  
}  ##  Fin de la funci?n  SAsc2Fb
...
##  Estimacion directa de las proporciones poblacionales
SAcsDEp <- apply(sapply(lapply(SSrwV, ExtrE, rw=1, cl=seq(nProv)), 
                        Subt, SAproTrg)^2, 1, mean)

##  Estimaci?n univariada (sin distancia)
SAcsAEp <- apply(sapply(SSrwV, SAcsAb, trg=SAproTrg)^2,1,mean)

##  Estimacion univariada (distancia 2)
SAcsFEp2 <- apply(sapply(SSrwV, SAcsFb, trg=SAproTrg, mxD=2)^2,1,mean)

##  Estimacion Univariada (distancia 3)
SAcsFEp3 <- apply(sapply(SSrwV, SAcsFb, trg=SAproTrg, mxD=3)^2,1,mean)

##  Estimacion Bivariada (sin distancia, usando datos del 2001)  S# ESTA ES LA MEJOR
SAcs2AEp <- apply(sapply(SSrwV, SAcs2Ab, Aux=Llar01[,1]/apply(Llar01,1,sum), 
                         trg=SAproTrg, tkn=0.00001)^2,1,mean)

##  Estimacion Bivariada (con distancia, usando datos del 2001)
SAcs2FEp2 <- apply(sapply(SSrwV, SAcs2Fb,Aux=Llar01[,1]/apply(Llar01,1,sum), 
                          trg=SAproTrg, mxD=2, tkn=0.00001)^2,1,mean)
...
for (i1 in seq(nEst-1))
{
  for (i2 in seq(i1, nEst)) 
  {
    ...
    SAcsResP[i2,i1] <- paste(round(res[[3]],3), "  [", sum(res[[2]]<1), "]",sep="")
    SAcsResP[i1,i2] <- paste(round(res[[4]][1],4), "  (", round(res[[4]][2],4), ")", sep="")
  }}  ##  Fin del doble loop
...
##  Grafico de comparaci?n directa de los EMCs de los 6 estimadores
##  Estimacion de la proporcion de indigenas  
...
{
  segments(nDs, x, nDs, y, lwd=0.5)
  points(nDs, y, pch=16, cex=0.45)
  
  if (length(vlin)>0) 
  {
    #     segments(Vlin, -0.025, Vlin, 0.025, lwd=0.4)
    #     segments(Vlin, 0.225, Vlin, 0.275, lwd=0.4)
...
graphics.off()
setwd(dir.resul)
postscript("Fig6Ap1.ps", width=5.5, height=5.8, pointsize=8, 
           horizontal=F) 
...
# dist(t(SAcs),diag=TRUE,upper=TRUE)
...
setwd(dir.dat)
datosE10 <- read.csv("datosENEMDU10.csv",sep=";",header=TRUE) 
datosE10$Cod <- poligonos@data$DPA_PROVIN

# Solo datos:
LlarE10 <- datosE10[,3:10]
rownames(LlarE10) <- AcrProv

# Matriz de entrada para estimaci?n:
Sprop <- LlarE10[,1]/apply(LlarE10,1,sum)
Smean <- rep(1,nProv)
Svar <- rep(0,nProv)
Ssize <- apply(LlarE10,1,sum)  # Chequear si es mejor poner el n?mero de observaciones de la muestra
...
SAcs2Ab <- function(drE, Aux=Llar01[,1]/apply(Llar01,1,sum), trg=SAproTrg, tkn=0)
{
  ##  Estimacion compuesta bivariada (simetr?a compuesta)
  ##  Estimacion de la media nacional
  ##    drE     Estimadores directos
  ##    Aux    Informacion auxiliar (asumiendo la variancion de la muestra)
  ##    trg     Los objetivos
  ...
  Esti - trg
  ## Resultados
  list(MSE = c(Esti - trg), Esti = Esti, est.dir = drE[1,], target = trg)
}  ##  Fin de la funcion  SAsc2Ab


##  Estimacion Bivariada (sin distancia, usando datos del 2001)  # ESTA ES LA MEJOR

ResEsti <- SAcs2Ab(LlarH10, Aux=Llar01[,1]/apply(Llar01,1,sum), trg=SAproTrg, tkn=0.00001)

Compara <- cbind(ResEsti$Esti,ResEsti$est.dir,ResEsti$target)
colnames(Compara) <- c("2-Comp-1","Directo","Objetivo")

Comp <- (Compara[,3]-Compara[,1])^2
Dir <- (Compara[,3]-Compara[,2])^2
CompV(Comp,Dir)
CompV(Compara[,3],Compara[,1])
CompV(Compara[,3],Compara[,2])

AmaComp <- sum(abs(Compara[c(14,15,16,19,20,21),3]-Compara[c(14,15,16,19,20,21),1]))
AmaDir <- sum(abs(Compara[c(14,15,16,19,20,21),3]-Compara[c(14,15,16,19,20,21),2]))
DirComp <- sum(abs(Compara[c(14,15,16,19,20,21),2]-Compara[c(14,15,16,19,20,21),1]))
abs(AmaComp-AmaDir)*100



AmaComp1 <- (abs(Compara[c(14,15,16,19,20,21),3]-Compara[c(14,15,16,19,20,21),1]))
AmaDir1 <- (abs(Compara[c(14,15,16,19,20,21),3]-Compara[c(14,15,16,19,20,21),2]))
