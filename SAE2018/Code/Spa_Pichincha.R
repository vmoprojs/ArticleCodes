...
############## Subconjunto de los cantones de Pichincha:
poligonos <- subset(poligonos,poligonos@data$DPA_CANTON=="1701" |
                      poligonos@data$DPA_CANTON=="1702" | poligonos@data$DPA_CANTON=="1703" |
                      poligonos@data$DPA_CANTON=="1704" | poligonos@data$DPA_CANTON=="1705" |
                      poligonos@data$DPA_CANTON=="1707" |poligonos@data$DPA_CANTON=="1708" |
                      poligonos@data$DPA_CANTON=="1709")
...
AcrProv <- c("QU","CA","ME","PM","RU","SA","PV","PQ")

graphics.off()

postscript("MapaAp1Pi.ps", width=5.5+2, height=3.9+2, pointsize=8, 
           horizontal=FALSE)
plot(poligonos)
...
# ProVec: CatVei
ProVec<-c("QUITO",  "QU",	"ME",	"PQ",				
          "CAYAMBE",	"CA",	"PM",	"PQ",				
          "MEJIA",	"ME",	"QU",	"PQ",				
          "PEDRO MONCAYO",	"PM",	"PQ",	"CA",				
          "RUMINAHUI",	"RU",	"PQ",	"SA",	"PV",			
          "SAN MIGUEL DE LOS BANCOS",	"SA",	"PV",	"RU",	"PQ",			
          "PEDRO VICENTE MALDONADO",	"PV",	"SA",	"RU",				
          "PUERTO QUITO",	"PQ",	"CA",	"PM",	"QU",	"ME",	"RU",	"SA")
...
setwd(dir.dat)
datos <- read.csv("datosCPV10Pichincha.csv",header=TRUE,sep=";")
...
setwd(dir.resul)
jpeg("PropIndMapPich.jpeg",quality=100,height = 600,width = 800)
...
graphics.off()
postscript("MapaPichInd.ps", width=5.5+2, height=3.9+2, pointsize=8, 
           horizontal=FALSE)
plot (poligonos.data, col=colcode, border="grey", axes=T)
# title(main = "Proporcion de Indigenas",cex=3)
legend("bottomright",legend = names(attr(colcode,"table")),
...
postscript("Fig2Ap1Pich.ps", width=5.5, height=3.9, pointsize=8, 
           horizontal=FALSE)
...
barplot((LlarC[,1]),space=0,col=rainbow(nProv),
        xlab="Cantones", ylab="Numero Indigenas",cex.names=0.6)
barplot(LlarH1,space=0,col=rainbow(nProv),
        xlab="Cantones", ylab="Porcentaje de Indigenas",cex.names=0.6)
...
NParc <- function(nD=round(CatC07/4000), B2=seq(0,0.03,length=101), B2m=0.02)
{
  ...
  ##  B2m   El sesgo al cuadrado m?s grande posible (posibles varios valores)
  
  
  ##  Puntos para el c?lculo y grafica
  B2 <- sort(unique(B2))
  
  ##  Numero de distritos y tama?o de muestra total
  D <- length(nD)
  nDs <- sum(nD)
...
CompV <- function(vc1, vc2, dgt=5, plt=T, smb=ProAcr)
{
  ...
  ##   Uso princial:  salida de la funci?n SAcs
  ##   vc1,  vc2    vectores
  ##   dgt      N?mero de d?gitos para el redondeo 
  ##   plt      L?gico, TRUE si se quiere plotear
  ##   smb      S?mbolos usados
...
##  Estimaci?n de area pequena para las proporciones de ind?genas
...
SAcsAb <- function(drE, trg=SAproTrg)
{
  ...
  ##    drE     Estimadores directos y tama?os muestrales
  ##    trg     Los objetivos
...
setwd(dir.dat)
datos01 <- read.csv("datosCPV01Pichincha.csv",sep=";",header=TRUE)
...
{
  ##  Estimacion compuesta  (simetr?a compuesta)
  ##  Estimacion de la media nacional
  ##    drE     Estimadores directos
  ##    Aux    Variable auxiliar
  ##    trg      Los objetivos
  ##    Psz     Tamanos de la poblacion por distrito
...
SAcs2AEp <- apply(abs(sapply(SSrwV, SAcs2Ab, Aux=Llar01[,1]/apply(Llar01,1,sum), 
                         trg=SAproTrg, tkn=0.00001)),1,mean)

##  Estimacion Bivariada (con distancia, usando datos del 2001) # Mejor para enemdu
SAcs2FEp2 <- apply(abs(sapply(SSrwV, SAcs2Fb,Aux=Llar01[,1]/apply(Llar01,1,sum), 
...
##  Eficiencia del conjunto de estimadores para las proporciones de ?rea peque?a
...
##  Grafico de comparacion directa de los EMCs de los 6 estimadores
...
MySeg <- function(x,y, nDs=nds, xlim=xlim, ylim=ylim, Vlin=vlin+1)
{
  ...
  points(nDs, y, pch=16,  cex=abs(x-y)*100)
...
graphics.off()
setwd(dir.resul)
postscript("Fig6Ap1Pich.ps", width=5.5, height=5.8, pointsize=8, 
           horizontal=F) 
...
tiff("Fig6Pich.tif", width = 800, height = 800, compression="none")
par(mfrow=c(1,1), mar=c(3,2,2,2), mgp=c(3,1,0)*0.6, lab=c(3,3,1))
...
setwd(dir.dat)
datosE10 <- read.csv("datosENEMDU10Pichincha.csv",sep=";",header=TRUE,dec=",") 
...
# Matriz de entrada para estimacion:
...
colnames(Compara) <- c("2-Comp-2","Directo","Obetivo")
