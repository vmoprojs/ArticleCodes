...
# ***************** REGULAR SPACE 1.2
rm(list=ls())
setwd("~/Documents/Articles/Coding/STBEU/Data/time")
# mse<-function(x,a)  var(x)+(mean(x)-a)^2 # Table 12
mse<-function(x,a)  median(abs(x-a)) # Table 3


#*********** ST Discarded functions:
# mse<-function(x,a)  mean(abs(x-a))
# mse<-function(x,a)
# {
#   qq = quantile(x)
...
dat1 <- read.csv("regular_results_timeDE15.csv")
names(dat1)
...
# ***************** REGULAR SPACE 1.8
dat1 <- read.csv("regular_results_timeDE2.csv")
names(dat1)
# mean = 0;scale_s=1; scale_t=1;sill=1
names.param = c("scale_s","scale_t","sill")
...
dat1 <- read.csv("Irregular_results_timeDE15.csv")
names(dat1)
# mean = 0;scale_s=1; scale_t=1;sill=1
names.param = c("scale_s","scale_t","sill")
...
#********************************************** GNEITING REGULAR
# ***************** REGULAR SPACE 1.2
dat1 <- read.csv("regular_results_timeGN15.csv")
names(dat1)
# mean = 0;scale_s=1; scale_t=1;sill=1
names.param = c("scale_s","scale_t","sill")
param <- c(1.2/3,1.2/19,1)
param <- c(3.1/3,3.1/19,1) #ST3 maxdist =maxtime 3,4
